"""minGRU implementation per arXiv:2410.01201v3 Appendix B."""
...
class MinGRULayer(nn.Module):
    """One multi-head minGRU layer with a parallel-training recurrence."""
    ...
    def step(self, x_t: torch.Tensor, state: RNNState) -> tuple[torch.Tensor, RNNState]:
        """Update one token using the sequential minGRU recurrence."""
        batch = x_t.shape[0]
        h_prev = state.hidden
        if h_prev.dim() == 2:
    ...
    def forward(self, x: torch.Tensor, state: RNNState) -> tuple[torch.Tensor, RNNState]:
        x_norm = self.norm(x)
        update, candidate = self._project(x_norm)
        ...
        output = self.dropout(output)
        # Return only the final hidden state [B, H, D] for next chunk/step
        ...
        **kwargs: object,
        ...
        self.layers = nn.ModuleList(
            [
                nn.ModuleList(
                    [
                        MinGRULayer(hidden_dim, num_heads, dropout),
